...
#ifndef SHIELD_SYNC_FASTCDC_CHUNKER_H
#define SHIELD_SYNC_FASTCDC_CHUNKER_H
...
static const uint32_t FASTCDC_GEAR[] = {
...
class FastCDChunker {
    ...
    public:
         ...
         * 
         */
        FastCDChunker(uint64_t min_size, uint64_t avg_size, uint64_t max_size, 
            uint64_t read_size);
        ...
        uint32_t LoadDataFromFile(uint8_t* data, uint32_t size);

        // uint32_t LoadDataFromFile()

        uint32_t LoadDataFromBuf(uint8_t* input_buf, uint64_t input_size);
