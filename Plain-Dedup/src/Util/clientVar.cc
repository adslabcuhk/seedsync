...
void ClientVar::InitUploadBuffer() {
    // allocate container in heap
    _curContainer.body = (uint8_t*) malloc(MAX_CONTAINER_SIZE);
    _curContainer.containerID = (char*) malloc(CONTAINER_ID_LENGTH);
    _curContainer.currentSize = 0;
// #if (CONTAINER_META_SESSION == 1)
    _curContainer.metadata = (uint8_t*) malloc(MAX_META_SIZE);
    // cout<<"max meta size "<<MAX_META_SIZE<<endl;
    // _curContainer.metadata = (uint8_t*) malloc(73728);
    // _curContainer.metadata = (uint8_t*) malloc(MAX_CONTAINER_SIZE);
    _curContainer.currentMetaSize = 0;
// #endif
    // // debug
    // uint8_t check_meta[MAX_META_SIZE];
    // memcpy(_curContainer.metadata, check_meta, MAX_META_SIZE);
    ...
    _recipeWritePath.assign(recipePath_);
    _recipeWriteHandler.open(recipePath_, ios_base::out | ios_base::binary);
...
void ClientVar::DestroyUploadBuffer() {
    free(_curContainer.body);
    free(_curContainer.containerID);
    
    // // debug
    // uint8_t check_meta[MAX_META_SIZE];
    // memcpy(_curContainer.metadata, check_meta, MAX_META_SIZE);
// #if (CONTAINER_META_SESSION == 1)
    free(_curContainer.metadata);
// #endif
...
void ClientVar::InitRestoreBuffer() {
    ...
    for (size_t i = 0; i < CONTAINER_CAPPING_VALUE; i++) {
        ...
        // uint64_t max_size = (MAX_CONTAINER_SIZE) + (MAX_META_SIZE);
        // cout<<"req containerArray[i] size "<<(MAX_CONTAINER_SIZE) + (MAX_META_SIZE)<<endl;
        // cout<<"req containerArray[i] size "<<MAX_CONTAINER_SIZE + MAX_META_SIZE<<endl;
        // cout<<MAX_CONTAINER_SIZE_WITH_META<<endl;
        // cout<<max_size<<endl;
        // cout<<"max container size "<<(MAX_CONTAINER_SIZE)<<endl;
        // cout<<"max meta size "<<(MAX_META_SIZE)<<endl;
#endif
    ...
    cout<<"open recipe file "<<recipePath_<<endl;
