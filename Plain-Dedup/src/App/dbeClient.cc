...
int main(int argc, char* argv[]) {
    ...
    cryptoObj = new CryptoPrimitive(CIPHER_TYPE, HASH_TYPE, HMAC_TYPE);
    ...
    uint8_t sessionKey[CHUNK_HASH_HMAC_SIZE] = { 0 };
    ...
    cryptoObj->GenerateHMAC((uint8_t*)&fullName[0],
    ...
    switch (optType) {
        case UPLOAD_OPT: { // for upload operation
            ...
            tool::Logging(myName.c_str(), "upload input file name: %s\n",
                inputFile.c_str());
            input_file_hdl.open(inputFile, ios_base::in | ios_base::binary);
            if (!input_file_hdl.is_open()) {
                tool::Logging(myName.c_str(), "cannot open the input file: %s\n",
                    inputFile.c_str());
                exit(EXIT_FAILURE);
            }
            ...
            dataSenderObj->SetSessionKey(sessionKey, CHUNK_HASH_HMAC_SIZE);
            ...
            thTmp = new boost::thread(attrs, boost::bind(&Chunker::Chunking, chunkerObj, &input_file_hdl));
        ...
        case DOWNLOAD_OPT: {
            ...
            dataRetrieverObj->SetSessionKey(sessionKey, CHUNK_HASH_HMAC_SIZE);
