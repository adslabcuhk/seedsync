...
void DataSender::UploadLogin(string localSecret, uint8_t* fileNameHash) {
    ...
    uint8_t masterKey[CHUNK_HASH_HMAC_SIZE];
    cryptoObj_->GenerateHMAC((uint8_t*)&localSecret[0], localSecret.size(),
        masterKey);

    // header + fileNameHash + Enc(masterKey)
    SendMsgBuffer_t msgBuf;
    msgBuf.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + CHUNK_HASH_HMAC_SIZE + CHUNK_HASH_HMAC_SIZE);
    ...
    msgBuf.header->dataSize += CHUNK_HASH_HMAC_SIZE;
    cryptoObj_->SessionKeyEnc(cipherCtx_, masterKey, CHUNK_HASH_HMAC_SIZE,
        sessionKey_, msgBuf.dataBuffer + CHUNK_HASH_HMAC_SIZE);
    msgBuf.header->dataSize += CHUNK_HASH_HMAC_SIZE;
    ...
    while (true) {
        ...
        if (inputMQ_->Pop(tmpChunk)) {
            switch (tmpChunk.dataType) {
                case DATA_CHUNK: {
                    // cout<<"DATA_CHUNK"<<endl;
                    // this is a normal chunk
                    this->ProcessChunk(tmpChunk.chunk);
                    break;
                }
                case DATA_SEGMENT_END_FLAG: {
                    // this is the segment end flag
                    // TODO: process the segment end flag
                    // cout<<"DATA_SEGMENT_END_FLAG"<<endl;
                    // this is a segment end
                    this->ProcessSegmentEnd(tmpChunk.segmentHead);
                    break;
                }
                case RECIPE_END: {
                    cout<<"RECIPE_END"<<endl;
...
/**
 * @brief process the segment end
 * 
 * @param segmentEnd the segment end
 */
void DataSender::ProcessSegmentEnd(SyncSegmentHead_t& segmentEnd) {
    // first check the send chunk buffer
    if (sendChunkBuf_.header->currentItemNum != 0) {
        this->SendChunks();
    }

    // send the segment end (without session encryption)
    sendChunkBuf_.header->messageType = CLIENT_UPLOAD_SEGMENT_END;
    sendChunkBuf_.header->dataSize = sizeof(SyncSegmentHead_t);
    memcpy(sendChunkBuf_.dataBuffer, &segmentEnd,
        sizeof(SyncSegmentHead_t));
    if (!dataSecureChannel_->SendData(conChannelRecord_.second,
        sendChunkBuf_.sendBuffer, 
        sizeof(NetworkHead_t) + sendChunkBuf_.header->dataSize)) {
        tool::Logging(myName_.c_str(), "send the segment end error.\n");
        exit(EXIT_FAILURE);
    }
    return ;
}
