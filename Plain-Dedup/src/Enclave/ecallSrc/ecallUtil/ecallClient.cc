 ...
 */
EnclaveClient::EnclaveClient(uint32_t clientID, int indexType, int optType, string recipePath)
{
    // store the parameters
    _clientID = clientID;
    indexType_ = indexType;
    optType_ = optType;
    currentRecipePath_.assign(recipePath);
...
// #if (CONTAINER_META_SESSION == 1)
    _inContainer.meta = (uint8_t*) malloc(MAX_META_SIZE * sizeof(uint8_t));
    // _inContainer.meta = (uint8_t*) malloc(73728 * sizeof(uint8_t));
    // _inContainer.meta = (uint8_t*) malloc(MAX_CONTAINER_SIZE * sizeof(uint8_t));
    _inContainer.curMetaSize = 0;
// #endif
    // // debug
    // uint8_t check_meta[MAX_META_SIZE];
    // memcpy(_inContainer.meta, check_meta, MAX_META_SIZE);
