...
void Ecall_Init_Client(uint32_t clientID, int type, int optType,
    uint8_t* encMasterKey, void** sgxClient, const char* recipePath)
{
    // allocate a client resource inside the enclave
    string recipeStr(recipePath);
    EnclaveClient* newClient = new EnclaveClient(clientID, type, optType, recipeStr);
...
newClient->SetMasterKey(encMasterKey, CHUNK_HASH_HMAC_SIZE);
...
void Ecall_Enclave_Init(EnclaveConfig_t* enclaveConfig) {
    ...
    enclaveKey_ = (uint8_t*)malloc(sizeof(uint8_t) * CHUNK_HASH_HMAC_SIZE);
    indexQueryKey_ = (uint8_t*)malloc(sizeof(uint8_t) * CHUNK_HASH_HMAC_SIZE);
    ...
    if (readFileSize == 0) {
        ...
        sgx_read_rand(enclaveKey_, CHUNK_HASH_HMAC_SIZE);
        sgx_read_rand(indexQueryKey_, CHUNK_HASH_HMAC_SIZE);
    } else {
        // file exists
        firstBootstrap_ = false;
        uint8_t* readBuffer = (uint8_t*)malloc(CHUNK_HASH_HMAC_SIZE * 2);
        Ocall_ReadSealedData(ENCLAVE_KEY_FILE_NAME, readBuffer, CHUNK_HASH_HMAC_SIZE * 2);
        memcpy(enclaveKey_, readBuffer, CHUNK_HASH_HMAC_SIZE);
        memcpy(indexQueryKey_, readBuffer + CHUNK_HASH_HMAC_SIZE,
            CHUNK_HASH_HMAC_SIZE);
        free(readBuffer);
    }
