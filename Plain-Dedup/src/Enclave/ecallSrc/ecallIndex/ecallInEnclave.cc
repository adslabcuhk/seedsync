...
bool EcallInEnclaveIndex::PersistDedupIndex() {
    ...
    maxBufferSize = Enclave::sendChunkBatchSize_ * (CHUNK_HASH_HMAC_SIZE + sizeof(RecipeEntry_t));
    ...
    for (auto it = insideIndexObj_.begin(); it != insideIndexObj_.end(); it++) {
        memcpy(tmpBuffer + offset, it->first.c_str(), CHUNK_HASH_HMAC_SIZE);
...
bool EcallInEnclaveIndex::LoadDedupIndex() {
    size_t itemNum;
    string keyStr;
    keyStr.resize(CHUNK_HASH_HMAC_SIZE, 0);
    string valueStr;
    ...
    for (size_t i = 0; i < expectReadBatchNum; i++) {
        ...
        for (size_t item = 0; item < Enclave::sendChunkBatchSize_; 
            item++) {
            memcpy(&keyStr[0], tmpBuffer + offset, CHUNK_HASH_HMAC_SIZE);
            offset += CHUNK_HASH_HMAC_SIZE;
