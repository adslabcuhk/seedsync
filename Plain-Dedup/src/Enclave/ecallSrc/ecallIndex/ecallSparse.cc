...
void EcallSparseIndex::LoadManifiest(string segmentID,
    ...
    for (size_t i = 0; i < chunkAddressNum; i++) {
        memcpy(&tmpFp[0], manifestValue.c_str() + i * sizeof(BinValue_t), CHUNK_HASH_HMAC_SIZE);
        memcpy(&tmpAddress[0], manifestValue.c_str() + i * sizeof(BinValue_t) + CHUNK_HASH_HMAC_SIZE,
...
bool EcallSparseIndex::PersistHookIndex() {
    ...
    for (auto it = hookIndex_->begin(); it != hookIndex_->end(); it++) {
        hookIndexSize += CHUNK_HASH_HMAC_SIZE;
    ...
    size_t offset = 0;
    for (auto it = hookIndex_->begin(); it != hookIndex_->end(); it++) {
        memcpy(tmpIndexBuffer + offset, it->first.c_str(), CHUNK_HASH_HMAC_SIZE);
...
bool EcallSparseIndex::LoadHookIndex() {
    ...
    while (offset != sealedDataSize) {
        memcpy(&tmpChunkFp[0], sealedDataBuffer + offset, CHUNK_HASH_HMAC_SIZE);
        offset += CHUNK_HASH_HMAC_SIZE;
...
void EcallSparseIndex::ProcessOneSegment(Segment_t* segment, UpOutSGX_t* upOutSGX) {
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        if (this->IsHook(metaDataPtr->chunkHash)) {
            ...
            string hookFp((char*)metaDataPtr->chunkHash, CHUNK_HASH_HMAC_SIZE);
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        ...
        if (tmpFindResult != tmpDedupIndex.end()) {
            ...
            memcpy(tmpBinValue.chunkFp, &tmpChunkFp[0], CHUNK_HASH_HMAC_SIZE);
