...
void EcallFreqIndex::ProcessTailBatch(UpOutSGX_t* upOutSGX)
{
    ...
    if (inRecipe->recipeNum != 0) {
        ...
        if (mapIt == Enclave::fileRecipeNameToHMACIndex_.end()) {
            ...
            Enclave::Logging("Add Recipe Index", "Tail batch insert recipe path: %s, hmac: %s.\n", recipePath.c_str(), Enclave::convertBufferToHexString(tailHMACBuffer, CHUNK_HASH_HMAC_SIZE).c_str());
        } else {
            mapIt->second.push_back(hmacStr);
            Enclave::Logging("Add Recipe Index", "Tail batch update recipe path: %s, hmac: %s.\n", recipePath.c_str(), Enclave::convertBufferToHexString(tailHMACBuffer, CHUNK_HASH_HMAC_SIZE).c_str());
        }
#endif
    ...
    if (sgxClient->_inContainer.curSize != 0) {
        ...
        sgxClient->_inContainer.curSize = 0;
#if (CONTAINER_META_SESSION == 1)
        memcpy(upOutSGX->curContainer->metadata, sgxClient->_inContainer.meta,
            sgxClient->_inContainer.curMetaSize);
        upOutSGX->curContainer->currentMetaSize = sgxClient->_inContainer.curMetaSize;
        sgxClient->_inContainer.curMetaSize = 0;
#endif
    }
...
bool EcallFreqIndex::PersistDedupIndex()
{
    ...
    requiredBufferSize = sizeof(size_t) + itemNum * (CHUNK_HASH_HMAC_SIZE + sizeof(HeapItem_t));
    tmpBuffer = (uint8_t*)malloc(sizeof(uint8_t) * requiredBufferSize);
    memcpy(tmpBuffer + offset, &itemNum, sizeof(size_t));
    offset += sizeof(size_t);
    for (size_t i = 0; i < itemNum; i++) {
        memcpy(tmpBuffer + offset, &heapPtr[i]->first[0], CHUNK_HASH_HMAC_SIZE);
    ...
    itemNum = Enclave::fileRecipeNameToHMACIndex_.size();
    for (auto it : Enclave::fileRecipeNameToHMACIndex_) {
        requiredBufferSize += (it.first.size() + it.second.size() * CHUNK_HASH_HMAC_SIZE + 2 * sizeof(uint32_t));
    }
...
bool EcallFreqIndex::LoadDedupIndex()
{
    size_t itemNum;
    string tmpChunkFp;
    tmpChunkFp.resize(CHUNK_HASH_HMAC_SIZE, 0);
    ...
    HeapItem_t tmpItem;
    string tmpFp;
    tmpFp.resize(CHUNK_HASH_HMAC_SIZE, 0);
    for (size_t i = 0; i < itemNum; i++) {
        memcpy(&tmpChunkFp[0], tmpIndexBuffer + offset, CHUNK_HASH_HMAC_SIZE);
        offset += CHUNK_HASH_HMAC_SIZE;
    ...
    for (uint32_t i = 0; i < itemNum; i++) {
        ...
        for (uint32_t hmacIndex = 0; hmacIndex < recipeHMACNumber; hmacIndex++) {
            string recipeHMACStr((char*)tmpHMACBuffer + offset, CHUNK_HASH_HMAC_SIZE);
            offset += CHUNK_HASH_HMAC_SIZE;
    ...
    UpOutSGX_t* upOutSGX)
{
    ...
    // Enclave::Logging("in processOneBatch","\n");
    ...
    string tmpHashStr;
    tmpHashStr.resize(CHUNK_HASH_HMAC_SIZE, 0);
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        ...
        cryptoObj_->GenerateHMAC(recvBuffer + currentOffset,
            inQueryEntry->chunkSize, inQueryEntry->chunkHash);

        // copy the chunkHash to file recipe entry
        memcpy(inQueryEntry->chunkAddr.chunkHash, inQueryEntry->chunkHash, CHUNK_HASH_HMAC_SIZE);

        currentOffset += inQueryEntry->chunkSize;
        inQueryEntry++;
    }

    // Enclave::Logging("after hashgen","\n");

    {
        ...
        for (size_t i = 0; i < chunkNum; i++) {
            cmSketch_->Update(inQueryEntry->chunkHash, CHUNK_HASH_HMAC_SIZE, 1);
    ...
    // Enclave::Logging("after update freq","\n");

    {
        ...
        // Enclave::Logging("after top-k","\n");

        for (size_t i = 0; i < chunkNum; i++) {
            tmpHashStr.assign((char*)inQueryEntry->chunkHash, CHUNK_HASH_HMAC_SIZE);

            // Enclave::Logging("after top-k","\n");

            auto findRes = sgxClient->_localIndex.find(tmpHashStr);
            if (findRes != sgxClient->_localIndex.end()) {
                // Enclave::Logging("in if after top-k","\n");
                ...
                // Enclave::Logging("in else after top-k","\n");
                ...
                if (inQueryEntry->chunkFreq < minFreq) {
                    ...
                    // Enclave::Logging("in else after top-k, before indexenc","\n");
#if (ENC_MODE == 1)
                    cryptoObj_->IndexAESCMCEnc(cipherCtx, inQueryEntry->chunkHash,
                        CHUNK_HASH_HMAC_SIZE, Enclave::indexQueryKey_, outQueryEntry->chunkHash);
#endif

#if (Plain_MODE == 1)

                    memcpy(outQueryEntry->chunkHash, inQueryEntry->chunkHash, CHUNK_HASH_HMAC_SIZE);
#endif                    

                    // Enclave::Logging("in else after top-k, after indexenc","\n");
                    ...
                    // Enclave::Logging("in else after top-k, before check contains","\n");
                    ...
                    if (topKRes) {
                        ...
                        // Enclave::Logging("in else after top-k, before memcpy inqueryentry","\n");
                        ...
                        // Enclave::Logging("in else after top-k, memcpy inqueryentry","\n");

                    } else {
                        // it does not exist in the heap
                        // Enclave::Logging("in else after top-k, before indexenc inqueryentry to outentry","\n");

#if (ENC_MODE == 1)
                        cryptoObj_->IndexAESCMCEnc(cipherCtx, inQueryEntry->chunkHash, CHUNK_HASH_HMAC_SIZE,
                        ...
                        // Enclave::Logging("in else after top-k, indexenc inqueryentry to outentry","\n");
    ...
    // Enclave::Logging("before out query","\n");
    ...
    // Enclave::Logging("after out query","\n");
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        ...
        case DUPLICATE: {
            ...
            // // copy the chunkHash to file recipe entry
            // memcpy((char*)&inQueryEntry->chunkAddr.chunkHash,
            //     (char*)&inQueryEntry->chunkHash, CHUNK_HASH_HMAC_SIZE);
        ...
        case TMP_DUPLICATE: {
            ...
            // // copy the chunkHash to file recipe entry
            // memcpy((char*)&tmpQueryEntry->chunkAddr.chunkHash,
            //     (char*)&tmpQueryEntry->chunkHash, CHUNK_HASH_HMAC_SIZE);
        ...
        case UNIQUE: {
...
#if (Plain_MODE == 1)
                memcpy((uint8_t*)&inQueryEntry->chunkAddr, (uint8_t*)&outQueryEntry->chunkAddr, sizeof(RecipeEntry_t));
#endif
            ...
            case UNIQUE: {
                ...
                finesse_util_->ExtractFeature(rabin_ctx_, recvBuffer + currentOffset,
                    tmpChunkSize, inQueryEntry->features);

                // for (int chk = 0; chk < SUPER_FEATURE_PER_CHUNK; chk++) {
                //     Enclave::Logging("feature ","%d\t", inQueryEntry->features[chk]);
                // }
                // Enclave::Logging(" ","\n");

                this->ProcessUniqueChunk(&inQueryEntry->chunkAddr, inQueryEntry->features,
                ...
                memcpy((uint8_t*)&outQueryEntry->chunkAddr, (uint8_t*)&inQueryEntry->chunkAddr, sizeof(RecipeEntry_t));
#endif                
        ...
        case TMP_UNIQUE: {
            ...
            // Enclave::Logging("check chunk hash1", " ");
            // Ocall_PrintfBinary(tmpQueryEntry->chunkAddr.chunkHash, CHUNK_HASH_HMAC_SIZE);
            // Ocall_PrintfBinary(tmpQueryEntry->chunkHash, CHUNK_HASH_HMAC_SIZE);
        ...
        // have prepared the tmpChunkAddr with chunkHash in recipe entry
    ...
    // Enclave::Logging("after process chunk","\n");
    ...
    UpOutSGX_t* upOutSGX)
{
    ...
    tmpCipherHashStr.resize(CHUNK_HASH_HMAC_SIZE, 0);
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        ...
        Ocall_GetCurrentTime(&_startTime);
#endif
        cryptoObj_->GenerateHMAC(recvBuffer + currentOffset, tmpChunkSize,
...
#endif
        // update the sketch
        cmSketch_->Update((uint8_t*)&tmpHashStr[0], CHUNK_HASH_HMAC_SIZE, 1);

        // estimate the current frequency
        chunkFreq = cmSketch_->Estimate((uint8_t*)&tmpHashStr[0], CHUNK_HASH_HMAC_SIZE);
#if (SGX_BREAKDOWN == 1)
        Ocall_GetCurrentTime(&_endTime);
        ...
        if (chunkFreq < currentMinFreq) {
            ...
            cryptoObj_->IndexAESCMCEnc(cipherCtx, (uint8_t*)&tmpHashStr[0], CHUNK_HASH_HMAC_SIZE,
            ...
            if (mapIt == Enclave::fileRecipeNameToHMACIndex_.end()) {
                ...
                hmacListVec.push_back(hmacStr);
                Enclave::fileRecipeNameToHMACIndex_.emplace(recipePath, hmacListVec);
                Enclave::Logging("Add Recipe Index", "insert recipe path: %s, hmac: %s.\n", recipePath.c_str(), Enclave::convertBufferToHexString(tailHMACBuffer, CHUNK_HASH_HMAC_SIZE).c_str());
            } else {
                mapIt->second.push_back(hmacStr);
                Enclave::Logging("Add Recipe Index", "update recipe path: %s, hmac: %s.\n", recipePath.c_str(), Enclave::convertBufferToHexString(tailHMACBuffer, CHUNK_HASH_HMAC_SIZE).c_str());
            }
