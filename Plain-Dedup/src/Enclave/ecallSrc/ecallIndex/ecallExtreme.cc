...
void EcallExtremeBinIndex::ProcessOneBatch(SendMsgBuffer_t* recvChunkBuf, 
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        ...
        cryptoObj_->GenerateHMAC(recvBuffer + offset,
        ...
        curSegmentMetaPtr = segment->metadata + 
            segment->chunkNum;
        memcpy(curSegmentMetaPtr->chunkHash, &tmpHashStr[0], CHUNK_HASH_HMAC_SIZE);
        ...
        if (chunkHashVal < segment->minHashVal) {
            ...
            memcpy(segment->minHash, &tmpHashStr[0], CHUNK_HASH_HMAC_SIZE);
        }
...
void EcallExtremeBinIndex::ProcessOneSegment(Segment_t* segment, 
    ...
    tmpSegmentHash.resize(CHUNK_HASH_HMAC_SIZE, 0);
    string tmpSegmentMinHash;
    tmpSegmentMinHash.resize(CHUNK_HASH_HMAC_SIZE, 0);
    ...
    cryptoObj_->GenerateHMAC((uint8_t*)segment->metadata,
        chunkNum * sizeof(SegmentMeta_t), (uint8_t*)&tmpSegmentHash[0]);

    // find the min-hash of the segment
    tmpSegmentMinHash.assign((char*)segment->minHash, CHUNK_HASH_HMAC_SIZE);
    ...
    if (findResult != primaryIndex_->end()) {
        ...
        if (tmpSegmentHash.compare(string((char*)primaryValuePtr->segmentHash, CHUNK_HASH_HMAC_SIZE)) != 0) {
            // this segment is different from the previous segment
            // conduct deduplication of the current segment
            for (size_t i = 0; i < chunkNum; i++) {
                tmpChunkFp.assign((char*)metaDataPtr->chunkHash, CHUNK_HASH_HMAC_SIZE);
        ...
        memcpy(primaryValuePtr->segmentHash, &tmpSegmentHash[0], CHUNK_HASH_HMAC_SIZE);
...
bool EcallExtremeBinIndex::FetchBin(string& binAddress, 
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        ...
        tmpChunkFp.assign((const char*)tmpBinValue->chunkFp, CHUNK_HASH_HMAC_SIZE);
...
bool EcallExtremeBinIndex::UpdateBinStore(string& binAddress, 
    ...
    for (auto it = tmpDedupIndex.begin(); it != tmpDedupIndex.end(); it++) {
        ...
        memcpy(tmpBinValue.chunkFp, it->first.c_str(), CHUNK_HASH_HMAC_SIZE);
...
bool EcallExtremeBinIndex::LoadPrimaryIndex() {
    ...
    size_t entryNum = sealedDataSize / (CHUNK_HASH_HMAC_SIZE + sizeof(PrimaryValue_t));
    string minHashStr;
    minHashStr.resize(CHUNK_HASH_HMAC_SIZE, 0);
    string primaryValueStr;
    primaryValueStr.resize(sizeof(PrimaryValue_t), 0);
    for (size_t i = 0; i < entryNum; i++) {
        memcpy(&minHashStr[0], sealedDataBuffer + i * (CHUNK_HASH_HMAC_SIZE + sizeof(PrimaryValue_t)),
            CHUNK_HASH_HMAC_SIZE);
        memcpy(&primaryValueStr[0], sealedDataBuffer + i * (CHUNK_HASH_HMAC_SIZE + sizeof(PrimaryValue_t)) + CHUNK_HASH_HMAC_SIZE,
...
bool EcallExtremeBinIndex::PersistPrimaryIndex() {
    ...
    uint32_t primaryIndexSize = primaryIndex_->size() * (CHUNK_HASH_HMAC_SIZE + sizeof(PrimaryValue_t));
    ...
    for (auto it = primaryIndex_->begin(); it != primaryIndex_->end(); it++) {
        memcpy(tmpIndexBuffer + offset * (CHUNK_HASH_HMAC_SIZE + sizeof(PrimaryValue_t)),
            it->first.c_str(), CHUNK_HASH_HMAC_SIZE);
        memcpy(tmpIndexBuffer + offset * (CHUNK_HASH_HMAC_SIZE + sizeof(PrimaryValue_t)) + CHUNK_HASH_HMAC_SIZE,
