...
void EcallRecvDecoder::ProcRecipeBatch(uint8_t* recipeBuffer, size_t recipeNum, 
...
#if (Plain_MODE == 1)
    memcpy(sgxClient->_plainRecipeBuffer, recipeBuffer, recipeNum * sizeof(RecipeEntry_t));
#endif    

    // Enclave::Logging("deb master key ", "\n");
    // Ocall_PrintfBinary(masterKey, CHUNK_HASH_HMAC_SIZE);
    // Enclave::Logging("recipeNum ", "%d\n", recipeNum);
    // uint8_t check_hash[CHUNK_HASH_HMAC_SIZE];
    EVP_MD_CTX* md_ctx = sgxClient->_mdCtx;
    // cryptoObj_->GenerateHMAC( recipeBuffer, recipeNum * sizeof(RecipeEntry_t),
    // check_hash);
    // Ocall_PrintfBinary(check_hash, CHUNK_HASH_HMAC_SIZE);
    // Enclave::Logging("sizeof ", "%d, %d \n", sizeof(RecipeEntry_t),
    //     recipeNum * sizeof(RecipeEntry_t));

    RecipeEntry_t* tmpRecipeEntry;
    tmpRecipeEntry = (RecipeEntry_t*)sgxClient->_plainRecipeBuffer;

    uint64_t tst_total_size = 0;
    uint64_t tst_total_send = 0;
    ...
    for (size_t i = 0; i < recipeNum; i++) {
        ...
        tst_total_size += tmpRecipeEntry->length;

        // Enclave::Logging("offset ", "%d\n", tmpRecipeEntry->offset);
        // Enclave::Logging("len ", "%d\n", tmpRecipeEntry->length);
        ...
        if (reqContainer->idNum == CONTAINER_CAPPING_VALUE) {
            ...
            for (size_t idx = 0; idx < sgxClient->_enclaveRecipeBuffer.size(); idx++) {
                ...
                memcpy((char*)&meta_offset, containerArray[containerID], sizeof(uint32_t));
                // Enclave::Logging("get meta offset = ", "%d\n", meta_offset);
                ...
                uint8_t* chunkBuffer = containerArray[containerID] + offset + meta_offset;
                // this->RecoverOneChunk(chunkBuffer, chunkSize, restoreChunkBuf, cipherCtx);
                this->RecoverOneChunk(chunkBuffer, chunkSize, restoreChunkBuf, cipherCtx, md_ctx);
                if (restoreChunkBuf->header->currentItemNum % 
                    ...
                    tst_total_send += restoreChunkBuf->header->dataSize;
    ...
    // cout<<"tst_total_size "<<tst_total_size<<endl;
    // Enclave::Logging("tst_total_size ", "%d\n", tst_total_size);
    // Enclave::Logging("tst_send_size ", "%d\n", tst_total_send);
...
#if (Plain_MODE == 1)
    memcpy(decompressedChunk, chunkBuffer, chunkSize);
#endif
...
void EcallRecvDecoder::RecoverOneChunk(uint8_t* chunkBuffer, uint32_t chunkSize, 
    SendMsgBuffer_t* restoreChunkBuf, EVP_CIPHER_CTX* cipherCtx, EVP_MD_CTX* md_ctx) {
    uint8_t* iv = chunkBuffer + chunkSize; 
    uint8_t* outputBuffer = restoreChunkBuf->dataBuffer + 
        restoreChunkBuf->header->dataSize;
    uint8_t decompressedChunk[MAX_CHUNK_SIZE];
    ...
    if (decompressedSize > 0) {
        ...
        // cryptoObj_->GenerateHMAC( decompressedChunk, chunkSize,
        //     check_hash);
        // Ocall_PrintfBinary(check_hash, CHUNK_HASH_HMAC_SIZE);
