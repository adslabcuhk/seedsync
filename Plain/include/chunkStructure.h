...
typedef struct {
   ...
   uint32_t minHashVal;
   uint8_t minHash[CHUNK_HASH_HMAC_SIZE];
   uint8_t* buffer;
...
typedef struct {
    ...
    uint32_t offset;
    uint32_t length;
    uint8_t chunkHash[CHUNK_HASH_HMAC_SIZE]; // add the chunkHash in file recipe entry (for segHash computation)
} RecipeEntry_t;
...
typedef struct {
    uint8_t segmentHash[CHUNK_HASH_HMAC_SIZE];
...
// typedef struct {
//     uint8_t sendBuffer[sizeof(NetworkHead_t) * 3 + MAX_CHUNK_SIZE];
// } Msg_t;
...
typedef struct {
    uint8_t fileName[CHUNK_HASH_HMAC_SIZE * 2];
} phase1MQ_t;

typedef phase1MQ_t phase2MQ_t;

typedef struct {
    uint8_t segHash[CHUNK_HASH_HMAC_SIZE];
    uint8_t is_find_end; 
} phase3MQ_t;
...
//     union {
//         uint8_t chunkData[MAX_CHUNK_SIZE];
//         uint8_t hash[CHUNK_HASH_HMAC_SIZE * 2]; // [chunk hash || base hash]
//     };
//     int dataType; // UNI_DATA or BASE_HASH
// } phase7MQ_t;

// typedef struct {
//     uint8_t hash[CHUNK_HASH_HMAC_SIZE * 2]; // [chunk hash || base hash]
//     uint8_t tag; // UNI_CHUNK or DELTA_CHUNK
// } phase8MQ_t;
...
typedef struct {
    uint8_t recipeName[CHUNK_HASH_HMAC_SIZE * 2];
    uint64_t offset;
    uint64_t chunkNum;
} SegIndexValue_t;
...
// typedef struct {
//     uint32_t queryNum;
//     OutBaseQueryEntry_t* OutBaseQueryBase;
// } OutBaseQuery_t;
