...
struct timeval stream5_stime;
struct timeval stream5_etime;
...
StreamPhase5Thd::StreamPhase5Thd(PhaseSender* phase_sender_obj, MessageQueue<StreamPhase4MQ_t>* inputMQ, 
    ...
    // for send batch
    send_batch_buf_.sendBuffer = (uint8_t*)malloc(sizeof(NetworkHead_t) + sync_config.GetDataBatchSize() * (MAX_CHUNK_SIZE + sizeof(uint32_t) + sizeof(uint8_t) + CHUNK_HASH_HMAC_SIZE) * sizeof(uint8_t));
    cout << "phase-5 allocate " << sizeof(NetworkHead_t) + sync_config.GetDataBatchSize() * (MAX_CHUNK_SIZE + sizeof(uint32_t) + sizeof(uint8_t) + CHUNK_HASH_HMAC_SIZE) << endl;
    ...
    while (true) {
        ...
        if (inputMQ_->Pop(tmp_entry)) {
            if (tmp_entry.is_file_end == NOT_FILE_END) {
                ...
                send_batch_buf_.header->dataSize += sizeof(StreamPhase4MQ_t);
                ...
                cout<<"process time phase-5 "<<stream5_process_time_<<endl;
...
void StreamPhase5Thd::ProcessOneBatch() {
    gettimeofday(&stream5_stime, NULL);
    // do ecall
    // Ecall_Stream_Phase5_ProcessBatch(uint8_t* in_buf, uint32_t in_size, 
    // ReqContainer_t* req_container, OutChunkQuery_t* addr_query, uint8_t* out_buf, 
    // uint32_t out_size)
    // sgx_status_t ret1 = Ecall_Test_Execute_2(sgx_eid_);

    // cout<<"111"<<endl;

    // Ecall_Test_Execute(sgx_eid_, recv_buf, recv_size, 
    //     &req_containers_, &out_chunk_query_, container_buf_, &update_index_);

    // cout<<"222"<<endl;

    // Ecall_Stream_Phase5_ProcessBatch(sgx_eid_, send_batch_buf_.dataBuffer, 
    ...
    gettimeofday(&stream5_etime, NULL);
    stream5_process_time_ += tool::GetTimeDiff(stream5_stime, stream5_etime);
 ...
 * @param out_size 
 */
void StreamPhase5Thd::ProcessPlainBatch(uint8_t* in_buf, uint32_t in_size, 
    ...
    for (size_t i = 0; i < in_num; i++) {
        ...
        if (get_hash_entry->sim_tag == SIMILAR_CHUNK || 
            ...
            memcpy(tmp_query_entry->chunkHash, get_hash_entry->chunkHash, CHUNK_HASH_HMAC_SIZE);
            ...
            //     CHUNK_HASH_HMAC_SIZE, SyncEnclave::index_query_key_, tmp_query_entry->chunkHash);
            memcpy(tmp_query_entry->chunkHash, get_hash_entry->baseHash, CHUNK_HASH_HMAC_SIZE);
    ...
    sync_io_->SyncIO_QueryChunkAddr((void*)addr_query);
    ...
    memcpy(out_buf, plain_in_buf_, out_offset_);
 ...
 * @param input_size 
 */
void StreamPhase5Thd::PocessSimChunk(uint8_t* base_chunk_data, uint32_t base_size,
    ...
    if (!this->DeltaEncode(base_chunk_data, base_size, sim_chunk_data, sim_size,
        ...
        tool::Logging("delta encode fails", "\n");
        ...
        memcpy(plain_in_buf_ + out_offset_, sim_chunk_data, sim_size);
 ...
 * @param input_size 
 */
void StreamPhase5Thd::ProcessNonSimChunk(uint8_t* input_chunk, uint32_t input_size) {
    ...
    memcpy(plain_in_buf_ + out_offset_, input_chunk, input_size);    
 ...
 * @return false 
 */
bool StreamPhase5Thd::DeltaEncode(uint8_t* base_chunk, uint32_t base_size,
    ...
    memcpy(plain_base, base_chunk, base_size);
    ...
    memcpy(plain_similar, input_chunk, input_size);
