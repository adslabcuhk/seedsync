...
void SyncOutEnclave::Destroy() {
    // pthread_rwlock_destroy(&seg_index_lck_);
...
void Ocall_QueryFeatureIndex(void* out_feature_query) {
    ...
    bool res = false;
    string base_hash;
    base_hash.resize(CHUNK_HASH_HMAC_SIZE, 0);
        ...
        // find base chunk
        {
            ...
            if (is_similar) {
                ...
                if (tmp_freq_vec[0].second == 1) {
                    memcpy(tmp_entry->baseHash, first_match_base_hash, CHUNK_HASH_HMAC_SIZE);
                }
                else {
                ...
                for (size_t i = 0; i < SUPER_FEATURE_PER_CHUNK; i++) {
                    out_feature_index_->InsertBothBuffer((char*)&tmp_entry->features[i],
                        sizeof(uint64_t), (char*)tmp_entry->baseHash, CHUNK_HASH_HMAC_SIZE);
                }
            }
