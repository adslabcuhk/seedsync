...
void Ecall_Sync_Enclave_Init(SyncEnclaveConfig_t* enclave_config)
{
    ...
    enclave_key_ = (uint8_t*)malloc(sizeof(uint8_t) * CHUNK_HASH_HMAC_SIZE);
    index_query_key_ = (uint8_t*)malloc(sizeof(uint8_t) * CHUNK_HASH_HMAC_SIZE);
    ...
    if (read_size == 0) {
        ...
        // key file exists
        uint8_t* read_buf = (uint8_t*)malloc(CHUNK_HASH_HMAC_SIZE * 2);
        Ocall_ReadSealedData(ENCLAVE_KEY_FILE_NAME, read_buf,
            CHUNK_HASH_HMAC_SIZE * 2);
        memcpy(enclave_key_, read_buf, CHUNK_HASH_HMAC_SIZE);
        memcpy(index_query_key_, read_buf + CHUNK_HASH_HMAC_SIZE, CHUNK_HASH_HMAC_SIZE);
        free(read_buf);
    }
