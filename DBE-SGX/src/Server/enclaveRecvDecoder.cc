...
void EnclaveRecvDecoder::GetReqContainers(ClientVar* outClient) {
    ...
    for (size_t i = 0; i < idNum; i++) {
        ...
        // size_t check_num = check_metasize / (CHUNK_HASH_SIZE + SUPER_FEATURE_PER_CHUNK * sizeof(uint64_t));
        ...
        //     memcpy(check_hash, containerArray[i] + read_meta_offset, CHUNK_HASH_SIZE);
        ...
        //     // // memcpy((char*)check_feature, (char*)containerArray[i] + sizeof(uint32_t) + k * (CHUNK_HASH_SIZE + SUPER_FEATURE_PER_CHUNK * sizeof(uint64_t)), SUPER_FEATURE_PER_CHUNK * sizeof(uint64_t));
        //     // cout<<"feature: ";
        ...
        //     // //     + k * (CHUNK_HASH_SIZE + SUPER_FEATURE_PER_CHUNK * sizeof(uint64_t)), CHUNK_HASH_SIZE);
        //     // tool::PrintBinaryArray(check_hash, CHUNK_HASH_SIZE);
        // }
