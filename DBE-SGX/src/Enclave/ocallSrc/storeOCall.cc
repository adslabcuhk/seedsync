...
// #else
    ...
    dataWriterObj_->SaveToFileWithMeta(&outClientPtr->_curContainer);
#endif
...
#if (CONTAINER_META_SESSION == 1)
    outClientPtr->_curContainer.currentMetaSize = 0;
#endif
    return;
}
...
void Ocall_WriteSealedFile(bool* ret, const char* sealedFileName, uint8_t* sealedDataBuffer, size_t sealedDataSize)
{
    fstream outSealedFile;
    outSealedFile.open(sealedFileName, ios_base::out | ios_base::binary);
    if (!outSealedFile.is_open()) {
        tool::Logging(myName_.c_str(), "cannot open the sealed file for recipe HMAC.\n");
        *ret = false;
        return;
    }
    outSealedFile.write((char*)sealedDataBuffer, sealedDataSize);
    outSealedFile.close();
    *ret = true;
    return;
}
...
#if (CONTAINER_META_SESSION == 1)
    // read the container with meta session
    // we have differeniated in the following function
    enclaveRecvDecoderObj_->GetReqContainers(outClientPtr);
#endif
    return;
}
