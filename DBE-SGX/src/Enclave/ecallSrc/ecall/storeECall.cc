...
void Ecall_Enclave_Init(EnclaveConfig_t* enclaveConfig)
{
    ...
    bool loadRecipeIndexStatus = Enclave::LoadRecipeIndex();
    if (!loadRecipeIndexStatus) {
        Enclave::Logging("Enclave Init", "Load Recipe HMAC Index failed, it may not exist.\n");
    }
#endif
    return;
}
...
void Ecall_Enclave_Destroy()
{
    using namespace Enclave;
#if RECIPE_HMAC == 1 && RECIPE_HMAC_PERSIST == 1
    bool destoryRecipeIndexStatus = Enclave::PersistRecipeIndex();
    if (!destoryRecipeIndexStatus) {
        Ocall_SGX_Exit_Error("Persist Recipe HMAC Index failed.");
    }
#endif
    // check the status
