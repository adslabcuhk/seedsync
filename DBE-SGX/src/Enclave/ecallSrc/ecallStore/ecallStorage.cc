...
void EcallStorageCore::SaveChunk(char* chunkData, uint32_t chunkSize,
    ...
    if ((CRYPTO_BLOCK_SIZE + chunkSize + saveOffset + featureSize + CHUNK_HASH_SIZE + saveMetaOffset < MAX_CONTAINER_SIZE) && (featureSize + CHUNK_HASH_SIZE + saveMetaOffset < MAX_META_SIZE)) {
        // current container can store this chunk
        // copy data to this container
        memcpy(inContainer->buf + writeOffset, chunkData, chunkSize);
        writeOffset += chunkSize;
        ...
        memcpy(inContainer->meta + writeMetaOffset, chunkAddr->chunkHash, CHUNK_HASH_SIZE);
        // Enclave::Logging("inContainer: ", "copy %d to offset %d, after copy %d\n", CHUNK_HASH_SIZE, writeMetaOffset, writeMetaOffset + CHUNK_HASH_SIZE);
        // memcpy(inContainer->meta + writeMetaOffset, chunkfp, CHUNK_HASH_SIZE);
    ...
    inContainer->curMetaSize += CHUNK_HASH_SIZE;
