...
{
#if DEBUG_FLAG == 1
    Enclave::Logging("EcallCrypto", "Total processed HMAC number: %ld\n", hmacCounter_);
#endif
    if (iv_) {
        free(iv_);
    }
...
#if DEBUG_FLAG == 1
    hmacCounter_++;
    Enclave::Logging("EcallCrypto", "Current processed HMAC number: %ld\n", hmacCounter_);
#endif
    unsigned int length;
    ...
    uint8_t* key, uint8_t* ciphertext)
{
    ...
    // Enclave::Logging("IndexAESCMCEnc", "\n");
    ...
    uint8_t* key, uint8_t* dataBuffer)
{
    ...
    // Enclave::Logging("IndexAESCMCDec", "\n");
