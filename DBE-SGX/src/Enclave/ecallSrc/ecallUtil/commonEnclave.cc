...
bool Enclave::PersistRecipeIndex()
{
    ...
    for (auto it : Enclave::fileRecipeNameToHMACIndex_) {
        ...
        Enclave::Logging("Write Recipe Index", "recipe path: %s, recipe path size: %ld, HMAC number: %ld.\n", it.first.c_str(), recipePathSize, recipeHMACNumber);
    }
    bool persistenceStatus = false;
    Ocall_WriteSealedFile(&persistenceStatus, SEALED_HMAC, tmpBuffer, requiredBufferSize);
    if (persistenceStatus == false) {
        Enclave::Logging("Enclave", "Persist Recipe HMAC Index failed.\n");
        return false;
    }
    free(tmpBuffer);
    return true;
...
bool Enclave::LoadRecipeIndex()
{
    ...
    for (uint32_t i = 0; i < itemNum; i++) {
        ...
        Enclave::Logging("Load Recipe Index", "recipe path: %s, recipe path size: %ld, HMAC number: %ld.\n", recipePathStr.c_str(), recipePathSize, recipeHMACNumber);
