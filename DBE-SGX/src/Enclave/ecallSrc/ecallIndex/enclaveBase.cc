    ...
    uint8_t* chunkBuffer, uint32_t chunkSize, UpOutSGX_t* upOutSGX)
{
    ...
    // uint8_t tmp_checkhash[CHUNK_HASH_SIZE];
    // memcpy(tmp_checkhash, chunkAddr->chunkHash, CHUNK_HASH_SIZE);
    // Ocall_PrintfBinary(tmp_checkhash, CHUNK_HASH_SIZE);
    ...
    if (tmpCompressedChunkSize > 0) {
        ...
        // memcpy(check_hash, chunkAddr->chunkHash, CHUNK_HASH_SIZE);
