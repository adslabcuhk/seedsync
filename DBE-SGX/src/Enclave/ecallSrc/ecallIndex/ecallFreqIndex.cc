...
void EcallFreqIndex::ProcessTailBatch(UpOutSGX_t* upOutSGX)
{
    ...
    if (inRecipe->recipeNum != 0) {
        ...
        if (mapIt == Enclave::fileRecipeNameToHMACIndex_.end()) {
            ...
            Enclave::Logging("Add Recipe Index", "Tail batch insert recipe path: %s, hmac: %s.\n", recipePath.c_str(), Enclave::convertBufferToHexString(tailHMACBuffer, CHUNK_HASH_SIZE).c_str());
        } else {
    ...
    UpOutSGX_t* upOutSGX)
{
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        ...
        uint8_t tmpHashVal[CHUNK_HASH_SIZE];
        cryptoObj_->GenerateHash(recvBuffer + currentOffset, inQueryEntry->chunkSize, tmpHashVal);
#endif

        // Enclave::Logging("check chunk hash1", " ");
        // Ocall_PrintfBinary(inQueryEntry->chunkHash, CHUNK_HASH_SIZE);

        // copy the chunkHash to file recipe entry
        memcpy(inQueryEntry->chunkAddr.chunkHash, inQueryEntry->chunkHash, CHUNK_HASH_SIZE);
...
#endif                        
#if (INDEX_ENC == 0)
                    memcpy(outQueryEntry->chunkHash, inQueryEntry->chunkHash, CHUNK_HASH_SIZE);
#endif                    
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        ...
        case DUPLICATE: {
            ...
            //     (char*)&inQueryEntry->chunkHash, CHUNK_HASH_SIZE);
        ...
        case TMP_DUPLICATE: {
            ...
            //     (char*)&tmpQueryEntry->chunkHash, CHUNK_HASH_SIZE);
...
#if (INDEX_ENC == 0)
                memcpy((char*)&inQueryEntry->chunkAddr, (char*)&outQueryEntry->chunkAddr,
                    sizeof(RecipeEntry_t));
#endif
                // // copy the chunkHash to file recipe entry
                // memcpy((char*)&inQueryEntry->chunkAddr.chunkHash,
...
#if (INDEX_ENC == 0)
                memcpy((char*)&outQueryEntry->chunkAddr, (char*)&inQueryEntry->chunkAddr,
                    sizeof(RecipeEntry_t));
#endif                    
                // update the statistic
                _uniqueChunkNum++;
        ...
        case TMP_UNIQUE: {
            ...
            // Enclave::Logging("check chunk hash1", " ");
            // Ocall_PrintfBinary(tmpQueryEntry->chunkAddr.chunkHash, CHUNK_HASH_SIZE);
            // Ocall_PrintfBinary(tmpQueryEntry->chunkHash, CHUNK_HASH_SIZE);
    ...
    UpOutSGX_t* upOutSGX)
{
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        ...
        // use the extra hash function
        uint8_t tmpHashVal[CHUNK_HASH_SIZE];
        cryptoObj_->GenerateHash(recvBuffer + currentOffset, tmpChunkSize, tmpHashVal);
#endif
#if (SGX_BREAKDOWN == 1)
        Ocall_GetCurrentTime(&_endTime);
