...
void EcallExtremeBinIndex::ProcessOneBatch(SendMsgBuffer_t* recvChunkBuf, 
    ...
    for (size_t i = 0; i < chunkNum; i++) {
        ...
        cryptoObj_->GenerateHash(recvBuffer + offset, tmpChunkSize, tmpHashVal);
#endif
    ...
    // reset
    uint32_t sendBatchSize = Enclave::sendChunkBatchSize_;
    memset(recvBuffer, 0, sendBatchSize * sizeof(Chunk_t));
    memset(recvChunkBuf->dataBuffer, 0, sendBatchSize * sizeof(Chunk_t));
    return ;
}
...
void EcallExtremeBinIndex::ProcessOneSegment(Segment_t* segment, 
    ...
    // use the extra hash function
    uint8_t tmpHashVal[CHUNK_HASH_SIZE];
    cryptoObj_->GenerateHash((uint8_t*)segment->metadata,
        chunkNum * sizeof(SegmentMeta_t), tmpHashVal);
#endif
