...
void Configure::ReadConf(std::string path) {
    ...
    // segment configure
    maxSegSize_ = root.get<uint64_t>("SegmentConfig.maxSegSize_");
    avgSegSize_ = root.get<uint64_t>("SegmentConfig.avgSegSize_");
    minSegSize_ = root.get<uint64_t>("SegmentConfig.minSegSize_");
