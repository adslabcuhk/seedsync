...
EcallStreamBaseHash::EcallStreamBaseHash(LocalityCache* enclave_locality_cache) {
    ...
    plain_feature_fp_list_ = (uint8_t*)malloc(SyncEnclave::send_chunk_batch_size_ * (CHUNK_HASH_SIZE + SUPER_FEATURE_PER_CHUNK * sizeof(uint64_t)));
...
void EcallStreamBaseHash::ProcessBatch(uint8_t* feature_fp_list, size_t feature_num, 
    ...
    // SyncEnclave::Logging("streambasehash recv batch ", "%d\n", feature_num);

    crypto_util_->DecryptWithKey(cipher_ctx_, feature_fp_list, feature_num * (CHUNK_HASH_SIZE + SUPER_FEATURE_PER_CHUNK * sizeof(uint64_t)),
        session_key_, plain_feature_fp_list_);

#if (DEBUG_FLAG == 1)
    // memcpy(plain_feature_fp_list_, feature_fp_list, feature_num * (CHUNK_HASH_SIZE + SUPER_FEATURE_PER_CHUNK * sizeof(uint64_t)));
#endif
    ...
    for (size_t i = 0; i < feature_num; i++) {
        ...
        memcpy(in_entry.chunkHash, plain_feature_fp_list_ + read_offset, CHUNK_HASH_SIZE);
        read_offset += CHUNK_HASH_SIZE;
        ...
        if (is_sim) {
            ...
            memcpy(out_entry.chunkHash, in_entry.chunkHash, CHUNK_HASH_SIZE);

            _enclave_similar_num ++;
#if (DEBUG_FLAG == 1)
            // debug check the basehash here
            // Ocall_PrintfBinary(out_entry.baseHash, CHUNK_HASH_SIZE);
#endif            
        }
        else {
            // non-similar chunk for enclave cache
            
            // detect local sim within this batch here
            ...
            if (batch_sim) {
                ...
                _batch_simlar_num ++;
#if (DEBUG_FLAG == 1)
                // Ocall_PrintfBinary(out_entry.chunkHash, CHUNK_HASH_SIZE);
                // Ocall_PrintfBinary(out_entry.baseHash, CHUNK_HASH_SIZE);
#endif                
            }
            else {
    ...
    // SyncEnclave::Logging("streambasehash sent batch ", "%d\n", out_num);
...
bool EcallStreamBaseHash::LocalDetect(uint64_t* features, uint8_t* chunkHash, 
    ...
    if (is_similar) {
        ...
        for (size_t k = 0; k < SUPER_FEATURE_PER_CHUNK; k++) {
            string insert_hash;
            insert_hash.assign((char*)chunkHash, CHUNK_HASH_SIZE);
...
bool EcallStreamBaseHash::QueryLocalFeature(uint64_t feature, string& baseHash) {
    if (batch_feature_index_.find(feature) != batch_feature_index_.end()) {
        baseHash.assign(batch_feature_index_[feature].c_str(), CHUNK_HASH_SIZE);
        return true;
    }
