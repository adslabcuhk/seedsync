...
EcallStreamChunkHash::EcallStreamChunkHash() {
    ...
    plain_fp_list_ = (uint8_t*)malloc(SyncEnclave::send_meta_batch_size_ * CHUNK_HASH_SIZE);
...
void EcallStreamChunkHash::ProcessBatch(uint8_t* filename_buf, uint32_t currentBatchID, uint8_t* recipe_buf,
...
#if (DEBUG_FLAG == 1)        
        SyncEnclave::Logging(my_name_.c_str(), "Cannot find recipe HMAC in metadata, target recipe path: %s, current index size: %ld.\n", recipePath.c_str(), Enclave::fileRecipeNameToHMACIndex_.size());
#endif        
    } else {
        recipeHMACExist = true;
        uint32_t recipeHMACNumber = recipeHMACsVecIt->second.size();
        // SyncEnclave::Logging(my_name_.c_str(), "Find recipe HMAC in metadata, target recipe path: %s, recipe HMAC number: %d, current batch ID: %ld.\n", recipePath.c_str(), recipeHMACNumber, currentBatchID);
    }
#endif
    // decrypt the recipe batch
    uint8_t dec_key[CHUNK_HASH_SIZE];
    ...
    if (recipeHMACExist) {
        ...
        uint8_t currentRecipeBatchHMACBuffer[CHUNK_HASH_SIZE];
        crypto_util_->GenerateHMAC(plain_recipe_buf_, recipe_num * sizeof(RecipeEntry_t), currentRecipeBatchHMACBuffer);
        uint8_t storedRecipeBatchHMACBuffer[CHUNK_HASH_SIZE];
        memcpy(storedRecipeBatchHMACBuffer, &((recipeHMACsVecIt->second)[currentBatchID][0]), CHUNK_HASH_SIZE);
        if (memcmp(currentRecipeBatchHMACBuffer, storedRecipeBatchHMACBuffer, CHUNK_HASH_SIZE) != 0) {
#if (DEBUG_FLAG == 1)
            SyncEnclave::Logging(my_name_.c_str(), "Read recipe batch ID: %ld, HMAC mismatch, stored HMAC: %s, computed HMAC: %s\n", currentBatchID, Enclave::convertBufferToHexString(storedRecipeBatchHMACBuffer, CHUNK_HASH_SIZE).c_str(), Enclave::convertBufferToHexString(currentRecipeBatchHMACBuffer, CHUNK_HASH_SIZE).c_str());
#endif
        }
    }
#endif
    ...
    // check key
    Ocall_PrintfBinary(dec_key, CHUNK_HASH_SIZE);
#endif
    ...
    for (size_t i = 0; i < recipe_num; i++) {
        ...
        RecipeEntry_t* tmp_recipe_entry = (RecipeEntry_t*)(plain_recipe_buf_ + sizeof(RecipeEntry_t) * i);
        ...
        uint8_t check_chunk_hash[CHUNK_HASH_SIZE];
        memcpy(check_chunk_hash, tmp_recipe_entry->chunkHash, CHUNK_HASH_SIZE);
        Ocall_PrintfBinary(check_chunk_hash, CHUNK_HASH_SIZE);
#endif       
    ...
    crypto_util_->EncryptWithKey(cipher_ctx_, plain_fp_list_,
        recipe_num * CHUNK_HASH_SIZE, session_key_, chunk_hash_buf);
