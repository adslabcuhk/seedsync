...
EcallCrypto::EcallCrypto(int cipherType, int hashType)
{
    ...
    if ((hmacCtx_ = HMAC_CTX_new()) == NULL) {
        Ocall_SGX_Exit_Error("EcallCrypto: allocate hmac ctx fails");
    }
    if ((mdCtx_ = EVP_MD_CTX_new()) == NULL) {
        Ocall_SGX_Exit_Error("EcallCrypto: allocate md ctx fails");
    }
}
...
{
#if DEBUG_FLAG == 1
    SyncEnclave::Logging("EcallCrypto", "Total processed HMAC number: %ld\n", hmacCounter_);
#endif
    if (iv_) {
        free(iv_);
    }
 ...
 */
void EcallCrypto::GenerateHash(uint8_t* dataBuffer, const int dataSize, uint8_t* hash)
{
 ...
 */
void EcallCrypto::GenerateHMAC(uint8_t* dataBuffer, const int dataSize, uint8_t* hmac)
{
#if DEBUG_FLAG == 1
    hmacCounter_++;
    SyncEnclave::Logging("EcallCrypto", "Current processed HMAC number: %ld\n", hmacCounter_);
#endif
    unsigned int length;
    ...
    uint8_t* key, uint8_t* ciphertext)
{
    ...
    // SyncEnclave::Logging("IndexAESCMCEnc", "\n");
    ...
    uint8_t* key, uint8_t* dataBuffer)
{
    ...
    // SyncEnclave::Logging("IndexAESCMCDec", "\n");
